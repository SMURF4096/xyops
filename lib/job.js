		...
		for (var queue_id in queues) {
			...
			if (job_limit && job_limit.rate && job_limit.window) {
				if (!this.jobRateLimits[queue_id]) {
					this.jobRateLimits[queue_id] = { id: queue_id, count: 0, max: job_limit.rate, window: job_limit.window, expires: now + job_limit.window };
				}
			}
			...
			if (job_limit && rate.expires && (now >= rate.expires)) {
				rate.count = 0;
				rate.expires = now + job_limit.window;
