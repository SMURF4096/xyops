	...
	requireValidPluginData(params, callback) {
		...
		if (params.params) {
			...
			for (var idx = 0, len = params.params.length; idx < len; idx++) {
				...
				else if ((param.type == 'select') && (typeof(param.value) != 'string')) {
					// Select values contain the comma-separated menu definition, and the
					// first item also serves as the default value for existing Events.
